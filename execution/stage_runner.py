...
def _required_capability_fact(value, *, default_unknown_reason: str, not_applicable_reason: str) -> dict:
    fact=_fact(value,default_unknown_reason=default_unknown_reason)
    if fact['result']=='not_applicable':
        return {'result':'blocked','reason_codes':list(dict.fromkeys([not_applicable_reason,*fact['reason_codes']]))}
    return fact


def _stage_applicability(value, *, stage_id: str) -> dict:
    if value is None:
        return {'state':'applicable','reason_codes':[]}
    if not isinstance(value,Mapping):
        raise ValueError(f'{stage_id}: stage applicability must be a mapping')
    state=value.get('state')
    if state not in _STAGE_APPLICABILITY:
        raise ValueError(f'{stage_id}: invalid stage applicability: {state}')
    reasons=value.get('reason_codes',[])
    if not isinstance(reasons,list) or any(not isinstance(reason,str) or not reason for reason in reasons):
        raise ValueError(f'{stage_id}: stage applicability reason_codes must be non-empty strings')
    if state=='not_applicable' and not reasons:
        raise ValueError(f'{stage_id}: not_applicable stage requires a recorded reason')
...
def _candidate_stage_capability_facts(
    ...
    for software in software_candidates:
        ...
        for semantic in semantics:
            ...
            facts.append(_required_capability_fact(
                value,
                default_unknown_reason=f'capability_unknown:{software}:{semantic}',
                not_applicable_reason=f'required_capability_not_applicable:{software}:{semantic}',
            ))
...
def run_profile(profile: Mapping, *, capabilities: Mapping[str,Mapping] | None = None, stage_checks: Mapping[str,Mapping], capabilities_by_software: Mapping[str,Mapping] | None = None, dependency_states: Mapping[str,Mapping] | None = None, stage_applicability: Mapping[str,Mapping] | None = None) -> dict:
    ...
    stage_applicability=stage_applicability or {}
    ...
    for stage in stages:
        ...
        applicability=_stage_applicability(stage_applicability.get(sid),stage_id=sid)
        if applicability['state']=='not_applicable':
            output.append({
                'stage_id':sid,
                'checkpoint':stage.get('checkpoint'),
                'assessment_result':'not_applicable',
                'assessment_reason_codes':list(applicability['reason_codes']),
                'release_result':'not_applicable',
                'release_reason_codes':list(applicability['reason_codes']),
                'depends_on':list(deps),
                'dependency_limitations':[],
                'recommended_release_target':None,
            })
            releases[sid]='not_applicable'
            seen.add(sid)
            continue
        cap_facts=[]
        ...
        if check_fact['result']=='not_applicable':
            check_fact={
                'result':'blocked',
                'reason_codes':list(dict.fromkeys([
                    f'stage_check_not_applicable_for_applicable_stage:{sid}',
                    *check_fact['reason_codes'],
                ])),
            }
        local_semantics=[
        ...
        for semantic in local_semantics:
            cap_facts.append(_required_capability_fact(
                capabilities.get(semantic),
                default_unknown_reason=f'capability_unknown:{semantic}',
                not_applicable_reason=f'required_capability_not_applicable:{semantic}',
            ))
        if software_semantics and software_candidates:
            cap_facts.extend(_candidate_stage_capability_facts(
                software_semantics,
                software_candidates,
    ...
    elif all(r=='pass' for r in release_results):
        overall='pass'
