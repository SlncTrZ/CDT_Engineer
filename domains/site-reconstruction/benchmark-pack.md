...
5. BS-05: seal artifact, inspect a disposable copy, verify accepted hashes remain stable.

## Negative and scale ladder
Missing/cyclic XREF; wrong unit; nested/mirrored transform; truncated query; unapproved inferred elevation; mid-stage mutation failure; uncertain timeout; source/output hash drift. Exercise repeated instances at 20, 100 and 1,000 after correctness is established. Record elapsed time, peak resource use, longest UI block, tool count and response size; freeze acceptance budgets before performance runs.
...
Result manifest: run ID, fixture hashes, engine/application/build/contract versions, five pack versions, standards, measurements, receipts, limitations, sealed artifacts and reviewer verdict. Production remains blocked until BS-01–05 and negative recovery gates pass.
